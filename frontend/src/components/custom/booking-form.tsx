...
import { getUnavailableSlots } from '@/lib/actions/booking.actions';
import { currencySymbols } from '@/lib/constants';
import { Calendar } from '@/components/ui/calendar';
import { Skeleton } from '../ui/skeleton';
import { useBookings } from '@/hooks/use-bookings'; // Import the newly created hook
...
export function BookingForm({ service, user }: BookingFormProps) {
  ...
  const { createBookingOptimistic } = useBookings(); // Use the hook
  ...
  const dailyLimitReached = user ? user.dailyBookings >= 10 : false;
  const currencySymbol = user ? (currencySymbols[user.currency] || '₹') : '₹';
  ...
  const handleBooking = async (e: React.FormEvent) => {
    ...
    try {
        ...
        await createBookingOptimistic(bookingData);
