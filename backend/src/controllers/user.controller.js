...
export const registerUser = asyncHandler(async (req, res) => {
    ...
    const userExists = await User.findOne({ $or: [{ email: email.toLowerCase() }, { mobile }] });
    if (userExists) {
        res.status(409); throw new Error('User with this email or mobile number already exists');
    }
    const baseUsername = email.split('@')[0].toLowerCase().replace(/[^a-z0-9]/g, '');
    const finalUsername = `${baseUsername}${crypto.randomBytes(2).toString('hex')}`;
    
    const user = await User.create({ name, username: finalUsername, email, password, mobile });

    if (user) {
        const userResponse = user.toObject();
        delete userResponse.password;
        res.status(201).json({ user: userResponse, token: generateToken(user._id) });
    } else {
        res.status(500); throw new Error('User could not be created.');
    }
});
...
export const loginUser = asyncHandler(async (req, res) => {
    const { identifier, password } = req.body;
    if (!identifier || !password) {
        res.status(400); throw new Error('Please provide credentials.');
    }
    ...
    if (user && (await user.matchPassword(password))) {
        const userResponse = user.toObject();
        delete userResponse.password;
        res.json({ user: userResponse, token: generateToken(user._id) });
    } else {
        res.status(401);
        throw new Error('Invalid credentials');
    }
