...
// LRUCache is a separate package, if not installed run: npm install lru-cache
...
export const getCoordsForPincode = async (pincode, country = 'IN') => {
    ...
    try {
        ...
        const { data } = await axios.get(url, {
            headers: {
                // It's good practice to set a custom User-Agent for public APIs
        ...
        if (data && data.length > 0) {
            const location = {
                // IMPORTANT: The database expects [longitude, latitude]
